...
extension NexusFFmpeg on NexusService {
  Future<File?> assembleVideo(Directory framesDir, {File? coverVideo}) async {
    ...
    final dataVideo = File(
      '${tmpDir.path}/data_video_${DateTime.now().millisecondsSinceEpoch}.mp4',
    );

    // 1. Synthesize data video from frames
    final dataArgs =
        '-framerate 30 -i ${framesDir.path}/frame_%05d.png -c:v libx264 -pix_fmt yuv420p -y ${dataVideo.path}';
    ...
    final outputVideo = File(
      '${tmpDir.path}/nexus_final_${DateTime.now().millisecondsSinceEpoch}.mp4',
    );
    ...
    final concatArgs =
        '-i ${coverVideo.path} -i ${dataVideo.path} '
        '-filter_complex "[0:v][1:v]concat=n=2:v=1[v]" '
        '-map "[v]" -c:v libx264 -preset superfast -y ${outputVideo.path}';
    ...
    if (ReturnCode.isSuccess(concatRc)) {
      ...
      AppLogger.error(
        'FFmpeg error (concat): ${await concatSession.getOutput()}',
      );
